  ...
  const onSubmit = async (data) => {
    try {
      ...
      await Swal.fire({
        icon: 'success',
        title: 'Job posted',
        text: 'Your job listing has been added.'
      })
      navigate(`/job/${job.id}`)
    } catch (e) {
      await Swal.fire({
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
        ...
        <div>
          ...
          <textarea
            ...
            rows={4}
            className="w-full px-4 py-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none resize-y"
        ...
        <div className="flex gap-3 pt-2">
          <button
            type="submit"
            className="px-5 py-2.5 bg-indigo-600 text-white rounded-lg font-medium hover:bg-indigo-500 transition-colors"
          >
            Post job
          </button>
          <button
